            ...
            <div class="row item-row">
                @foreach($news as $new)
                    <div class="col-12 col-lg-6 item-col item-col-1">
                        <div>
                            <a href="{{route('photodiary')}}"><img class="news_image_intro" src="{{url('/')}}/{{$new->news_image_intro}}" alt=""></a>
                            @foreach($categories as $category)
                                @if($new->category_id == $category->id)
                                    <div class="item-div-1"><a class="item-a" href="{{route('photodiary')}}">{{$category->category_name}}</a></div>
                                @endif
                            @endforeach
    ...
    <section>
        <div class="section-loadmore">
            <div class="item-section-loadmore">
                <a href="{{route('photodiary.loadmore')}}">Load more</a>
            </div>
        </div>
    </section>
@endsection
